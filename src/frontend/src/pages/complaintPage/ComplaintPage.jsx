  ...
  const loadComplaintReplies = async (complaintId = null) => {
    try {
      ...
      if (response.data) {
        ...
        if (complaintId) {
          const repliesForThisComplaint = repliesArray.filter(
            (reply) => reply.id === complaintId
          );
        }
      } else {
        setComplaintReplies([]);
          ...
          <div className="complaint-detail-card">
            <div className="complaint-detail-header">
              <div className="complaint-meta">
                <h2>
                  #{selectedComplaint.id} - {selectedComplaint.problem}
                </h2>
              ...
              <div className="complaint-date">
                <Icons.Clock />
                {new Date(selectedComplaint.createAt).toLocaleDateString(
                  "vi-VN",
                  {
                    year: "numeric",
                    month: "long",
              ...
              <h3>
                ...
                Admin Responses (
                {
                  complaintReplies.filter(
                    (reply) => reply.id === selectedComplaint.id
                  ).length
                }
                )
              </h3>
              <div className="responses-list">
                {loadingReplies ? (
                      ...
                      <div key={index} className="response-item">
                        <div className="response-header">
                          ...
                          <span className="response-date">
                            {reply.resolveAt
                              ? new Date(reply.resolveAt).toLocaleDateString(
                                  "vi-VN",
                                  {
                                    year: "numeric",
                                    month: "short",
                        ...
                        <div className="response-content">{reply.solution}</div>
                      </div>
                    ))
                ) : (
                  <div className="no-responses">
                    <Icons.MessageCircle />
                    <p>
                      No admin responses yet. Your complaint is being reviewed.
                    </p>
                  </div>
                )}
              </div>
